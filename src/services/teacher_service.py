...
class TeacherService:
    ...
    def __init__(self, storage=None):
        ...
        self.teachers = self.storage.load_entities(Teacher, "profesores")

    def _save_changes(self):
        """Método privado para guardar cambios"""
        return self.storage.save_entities(self.teachers, "profesores")
    ...
    def add_teacher(self, name, specialty, email, salary=None):
        ...
        new_teacher = Teacher(new_id, name.strip(),
                              specialty.strip(), email.strip(), salary)
            ...
            if email is not None and email.strip():
                ...
                for other_teacher in self.teachers:
                    if other_teacher.id != teacher_id and other_teacher.email.lower() == email.strip().lower():
                        print(
                            f"❌ Ya existe otro profesor con el email: {email}")
                        return False
    ...
    def get_statistics(self):
        ...
        salaries = [
            teacher.salary for teacher in self.teachers if teacher.salary is not None]
        specialties = {}

        for teacher in self.teachers:
            specialties[teacher.specialty] = specialties.get(
                teacher.specialty, 0) + 1
